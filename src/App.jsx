...
function App() {
  ...
  const fetchData = async (userId) => {
    ...
    const cardsQuery = query(collection(db, 'cards'), where("userId", "==", userId), orderBy('order'));
  ...
  const onDragEnd = (result) => {
    ...
    const boardCards = cards.filter(c => c.boardId === selectedBoardId).sort((a, b) => a.order - b.order);
    const [reorderedItem] = boardCards.splice(source.index, 1);
    boardCards.splice(destination.index, 0, reorderedItem);
    const otherCards = cards.filter(c => c.boardId !== selectedBoardId);
    const batch = writeBatch(db);
    boardCards.forEach((card, index) => { const cardRef = doc(db, 'cards', card.id); batch.update(cardRef, { order: index }); });
    batch.commit();
    const updatedCardsInState = boardCards.map((card, index) => ({...card, order: index}));
    setCards([...otherCards, ...updatedCardsInState]);
  };
